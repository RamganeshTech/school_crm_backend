...
export const addExpense = async (req: RoleBasedRequest, res: Response) => {
    try {
        ...
        const billFiles = files && files["billProof"] ? files["billProof"] : [];
        const workFiles = files && files["workProof"] ? files["workProof"] : [];
        ...
        // 4. SAVE TO DATABASE
        // -------------------------------------------------
        const newExpense: any = new ExpenseModel({
...
export const getExpenseById = async (req: RoleBasedRequest, res: Response) => {
    try {
        const { id } = req.params;
...
export const getAllExpenses = async (req: RoleBasedRequest, res: Response) => {
    try {
        const {
            schoolId,
            expenseNo,
        ...
        // --- FILTER: REF ID / EXPENSE NO (Partial & Case Insensitive Match) ---
        if (expenseNo) {
        ...
        // 2 & 3. Fetch Data and Total Count in Parallel
...
export const deleteExpense = async (req: RoleBasedRequest, res: Response) => {
    try {
        const { id } = req.params;
...
export const updateExpense = async (req: RoleBasedRequest, res: Response) => {
    try {
        const { id } = req.params;
        let {
            schoolId,
        ...
        const expense: IExpense | null = await ExpenseModel.findById(id);
        if (!expense) {
        ...
        const newBillFiles = files && files["billProof"] ? files["billProof"] : [];
        const newWorkFiles = files && files["workProof"] ? files["workProof"] : [];
        ...
        if (uploadedBills.length > 0) {
            (expense.bill as any) = [...expense.bill, ...uploadedBills];
        }
        if (uploadedWorkPhotos.length > 0) {
            (expense.workPhoto as any) = [...expense.workPhoto, ...uploadedWorkPhotos];
        }
...
export const updateExpenseStatus = async (req: RoleBasedRequest, res: Response) => {
    try {
        const { id } = req.params;
...
export const deleteProof = async (req: RoleBasedRequest, res: Response) => {
    try {
        ...
        // 2. Find the Expense
        const expense: IExpense | null = await ExpenseModel.findById(expenseId);
        if (!expense) {
            return res.status(404).json({ ok: false, message: "Expense not found" });
        }
        ...
        const proofArray = expense[type as keyof IExpense] as any[]; // Access array dynamically
        ...
        const fileIndex = proofArray.findIndex((item: any) => item._id.toString() === proofId);
